...
void app_main(void)
{
    ...
    while (1)
    {
        ...
        // Encontrar el valor mínimo de la CDF 
        uint32_t cdf_min = 0;
        for (int i = 0; i < 256; i++) {
            if (cdf[i] > 0) {
                cdf_min = cdf[i];
        ...
        // Aplicar la fórmula de ecualización a cada píxel en el buffer
        // h(v) = round((cdf(v) - cdf_min) / (total_pixels - cdf_min) * 255)
        uint32_t total_pixels = pic->len;
        for (int i = 0; i < pic->len; i++) {
            uint8_t original_val = pic->buf[i];
            float equalized = (float)(cdf[original_val] - cdf_min) / (total_pixels - cdf_min) * 255.0;
            
            // Asegurar que el valor esté entre 0 y 255
            if (equalized < 0) equalized = 0;
            if (equalized > 255) equalized = 255;
            
            pic->buf[i] = (uint8_t)equalized;
        }
