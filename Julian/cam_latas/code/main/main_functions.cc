...
static bool InvokeCanDetector(const int8_t *frame, float scores[kCategoryCount]) {
  ...
  if (output->bytes < kCategoryCount) {
    MicroPrintf("Output tensor is smaller than expected for can detector.");
    return false;
  }
...
void setup() {
  ...
  model = tflite::GetModel(g_can_detect_model_data);
...
void loop() {
  ...
  if (!InvokeCanDetector(captured_frame, scores)) {
    vTaskDelay(1);
    return;
  }

  RespondToDetection(scores[kCanIndex], scores[kNotCanIndex]);
