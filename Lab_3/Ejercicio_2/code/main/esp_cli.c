...
static void run_static_images_auto(void)
{
#if CLI_ONLY_INFERENCE
    ESP_LOGI(TAG, "Running static images automatically");

    for (int i = 0; i < 4; i++) {
        ESP_LOGI(TAG, "Running detect_image %d", i);

        unsigned detect_time = esp_timer_get_time();
        run_inference((void *)image_database[i]);
        detect_time = (esp_timer_get_time() - detect_time) / 1000;

        ESP_LOGI(TAG, "Time required for image %d inference is %u ms", i, detect_time);
