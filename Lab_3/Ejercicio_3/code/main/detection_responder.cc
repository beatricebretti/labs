...
static bool last_person_detected = false;

void RespondToDetection(float person_score, float no_person_score) {
  int person_score_int = (person_score) * 100 + 0.5;
  (void) no_person_score;
  const bool person_detected = person_score_int >= 60;
  ...
  if (person_detected && !last_person_detected) {
    ESP_LOGI(TAG, "PERSON DETECTED! (score: %d%%)", person_score_int);
  } else if (!person_detected && last_person_detected) {
    ESP_LOGI(TAG, "No person detected (score: %d%%)", person_score_int);
  }
