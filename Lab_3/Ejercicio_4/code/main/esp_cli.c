...
#if CLI_ONLY_INFERENCE
static const char *expected_label_for_image(int image_number)
{
    return (image_number < 5) ? "identifier (con_identificador)" :
                                  "no_identifier (sin_identificador)";
}

void run_all_static_images(void)
{
    image_database_init();
    ESP_LOGI(TAG, "Running automatic inference on %d static images...", IMAGE_COUNT);

    while (true) {
        for (int i = 0; i < IMAGE_COUNT; i++) {
            ESP_LOGI(TAG, "--- image%d | expected: %s ---", i, expected_label_for_image(i));
            unsigned detect_time = esp_timer_get_time();
            run_inference((void *) image_database[i]);
            detect_time = (esp_timer_get_time() - detect_time) / 1000;
            ESP_LOGI(TAG, "image%d inference time: %u ms", i, detect_time);
            vTaskDelay(pdMS_TO_TICKS(2000));
        }
        ESP_LOGI(TAG, "Finished pass over all images, restarting in 5 s...");
